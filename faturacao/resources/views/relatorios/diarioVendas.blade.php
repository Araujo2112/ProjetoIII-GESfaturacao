...
<div class="bg-dark-subtle d-flex justify-content-center align-items-start min-vh-100 pt-5">
    <div class="bg-white rounded shadow p-4 mx-auto" style="width:100%; max-width:1400px; min-height:380px;">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="text-dark text-center m-0 flex-grow-1">Relatório - Diário</h1>

            <div class="d-flex gap-2">
                <button type="button" class="btn btn-outline-primary btn-sm" onclick="exportDiarioPdf()">
                    Exportar PDF
                </button>

                {{-- CSV mantém os filtros via querystring --}}
                <a class="btn btn-outline-secondary btn-sm"
                   href="{{ route('relatorios.diario.export.csv', request()->query()) }}">
                    Exportar CSV
                </a>
            </div>
        </div>
        ...
        <form method="GET" class="d-flex align-items-center mb-4" style="gap:1rem;" id="filtroForm">
            ...
            <div id="camposPersonalizado" class="d-flex align-items-center"
                 style="gap:0.5rem; {{ request('periodo') != 'personalizado' ? 'display:none;' : '' }}">
...
<script>
    ...
    // default
    window.diarioModo = 'lucro';

    async function exportDiarioPdf() {
        try {
            if (!window.diarioVendasChart) {
                alert('O gráfico ainda não está pronto. Tenta novamente em 1-2 segundos.');
                return;
            }

            const { imgURI } = await window.diarioVendasChart.dataURI();

            const periodo = document.getElementById('periodoSelect')?.value || 'mes';
            const di = document.querySelector('input[name="data_inicio"]')?.value || '';
            const df = document.querySelector('input[name="data_fim"]')?.value || '';

            const form = document.createElement('form');
            form.method = 'POST';
            form.action = "{{ route('relatorios.diario.export.pdf') }}";

            form.innerHTML = `
                <input type="hidden" name="_token" value="${window.csrfToken}">
                <input type="hidden" name="chart_img" value="${imgURI}">
                <input type="hidden" name="modo" value="${window.diarioModo}">
