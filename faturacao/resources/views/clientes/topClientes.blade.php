...
@section('content')
    <div class="bg-dark-subtle d-flex justify-content-center align-items-start min-vh-100 pt-5">
        <div class="bg-white rounded shadow p-4 mx-auto" style="width:100%; max-width:1400px; min-height:380px;">
            ...
            {{-- Gráfico/Tabela --}}
            @if(empty($top5ClientesVendas) || count($top5ClientesVendas) == 0)
                <div class="text-center py-5">
                    <h4 class="fw-semibold mt-4 mb-2">Sem dados em Clientes</h4>
    ...
    @if(!empty($top5ClientesVendas) && !empty($top5ClientesEuros))
        <script>
            window.topClientesData = {
                vendas: @json($top5ClientesVendas->pluck('num_vendas')->all()),
                euros: @json($top5ClientesEuros->pluck('total_euros')->all()),
                nomesVendas: @json($top5ClientesVendas->pluck('cliente')->all()),
...
@push('scripts')
    @vite(['resources/js/clientes/topClientes.js'])
@endpush
